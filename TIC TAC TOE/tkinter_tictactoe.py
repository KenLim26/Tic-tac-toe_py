...
def check_winner():
    ...
    for row in range(3):
        if board[row][0]["text"] == board[row][1]["text"] == board[row][2]["text"] and board[row][0]["text"] != "":
            ...
            for column in range(3):
                board[row][column].config(background=color_yellow, foreground=color_light_gray)
            game_over = True
            return
    ...
    if board[0][0]["text"] == board[1][1]["text"] == board[2][2]["text"] and board[0][0]["text"] != "":
        ...
        for row in range(3):
            board[row][row].config(background=color_yellow, foreground=color_light_gray)
        game_over = True
        return
    
    if (board[0][2]["text"] == board[1][1]["text"] == board[2][0]["text"] and board[0][2]["text"] != ""):
        label.config(text=board[0][2]["text"]+" wins!", foreground=color_yellow)
        board[0][2].config(background=color_yellow, foreground=color_light_gray)
        board[1][1].config(background=color_yellow, foreground=color_light_gray)
        board[2][0].config(background=color_yellow, foreground=color_light_gray)
        game_over = True
        return
...
button = tkinter.Button(frame, text="Reset", font=("Consolas", 20), background=color_gray, foreground="white", command=new_game)
